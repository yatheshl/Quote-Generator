  ...
  useEffect(() => {
    ...
    function setPallete() {
      ...
      for (var i = surfaces.length; i--; ) {
        surfaces[surfaces.length - i - 1].color =
          "#" + palletes[palleteNum][i + 1];
      }
    }
    ...
    function resizeSuraces() {
      ...
      totalPoints = Math.round(
        Math.clamp(Math.pow(Math.random() * 8, 2), 16, window.innerWidth / 35)
      );
      dist = Math.clamp(window.innerWidth / 4, 150, 200);
    ...
    function init() {
      ...
      window.onmousedown = wobbleSuraces;
    ...
    // Replace all instances of `get('canvas')` with `canvasRef.current`.
    // Replace all instances of `window.` with `windowRef.current.`.
    // ...
  }, []);
